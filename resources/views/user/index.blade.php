<div class="home">
    ...
    <swiper-slide>
        <h3>الشعبة : {{$filiere->nom_ar}}</h3>
        <h3>المكان : @foreach ($filiere->lieu as $lieu) {{$lieu->nom_ar}} - @endforeach</h3>
        <h3>@foreach ($lieu->date as $date) / {{$date->date}} @endforeach</h3>
        <a class="btn" href="{{url('prendre_rdv')}}">اختار موعدا</a>
    </swiper-slide>
    @endforeach
  </swiper-container>
</div>